  ...
  useEffect(() => {
    sseRef.current = createSSEConnection(
      (type, data) => {
        ...
        setTimeout(() => setAuctionMessages(prev => [...prev, msg]), 0);
      },
    ...
    <div className="grain min-h-screen" style={{ background: '#121212' }}>
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={fadeUp} transition={{ duration: 0.6 }}>
        <section id="settlement" style={{ paddingTop: 80 }}>
          <Settlement data={eliteResult?.phase_6_payments || null} />
        </section>
      </motion.div>
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} variants={fadeUp} transition={{ duration: 0.6 }}>
        ...
        {eliteResult && (
          <OrchestratorAnalysis data={eliteResult.phase_1_analysis} />
        )}
        {eliteResult && (
          <AgentWorkspace outputs={eliteResult.phase_3_all_outputs} />
        )}
        {eliteResult && (
          <ValidationVerdict
            data={{
              winner: eliteResult.phase_4_council_validation?.winner,
              quality_score: (() => {
                const v = eliteResult.phase_4_council_validation;
                return v?.quality_scores?.[v?.winner] ?? v?.quality_score ?? 0;
              })(),
              explanation: eliteResult.phase_4_council_validation?.explanation,
            }}
          />
        )}
        {eliteResult && (
          <Settlement data={eliteResult.phase_6_payments} />
        )}
